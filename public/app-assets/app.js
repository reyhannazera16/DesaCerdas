...
function drawCharts() {
  ...
  // Zones
  $(function() {
    var result = alasql("SELECT KELURAHAN AS label, COUNT(*) AS total FROM ? GROUP BY PRODUK", [features]);
  ...
  $(function() {
    var sizes = [];
    var KELURAHAN = alasql("SELECT PRODUK AS category, COUNT(*) AS total FROM ? WHERE CAST(PRODUK as INT)", [features]);
    ...
    sizes.push(KELURAHAN, sapling, small, medium, large, giant);
  ...
  // Species
  $(function() {
    var result = alasql("SELECT KELURAHAN AS LABEL, COUNT(*) LABEL FROM KELURAHAN WHERE PRODUK ORDER BY PRODUK ASC ", [features]);
    var chart = c3.generate({
